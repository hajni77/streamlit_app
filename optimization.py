...
def optimization(placed_obj, room_sizes):
    ...
    for i,obj in enumerate(placed_obj):
        ...
        # check the distance from the walls perendicular to the wall the object is on
            dist_front,dist_left,dist_right,dist_bottom = check_distance_from_wall((x,y,width,depth), room_width, room_depth, wall,shadow)
            # create dictionary
            dist = {"top":dist_front, "left":dist_left, "right":dist_right, "bottom":dist_bottom}
            ...
            if min_dist < 30:
                ...
                # convert values
                conv_x,conv_y,conv_width,conv_depth, conv_shadow_front, conv_shadow_left, conv_shadow_right, conv_shadow_back = convert_values((x,y,width,depth), shadow, wall)
                # adjust object placement
                new_x,new_y = adjust_object_placement_pos((conv_x,conv_y,conv_width,conv_depth), (conv_shadow_front, conv_shadow_left, conv_shadow_right, conv_shadow_back), room_width, room_depth, wall_side)
                # Create a new list excluding the current object
                other_positions = extracted_positions[:i] + extracted_positions[i+1:]
                
                if is_valid_placement((x,y,width,depth), other_positions, (conv_shadow_front, conv_shadow_left, conv_shadow_right, conv_shadow_back), room_width, room_depth):
...
def optimize_object (rect, shadow, room_width, room_depth, placed_obj):
    ...
    rect_front, rect_left, rect_right, rect_bottom = shadow
    ...
    for i,obj in enumerate(placed_obj):
        ...
        if rect_wall in wall or wall in rect_wall:
            ...
            if dist < 50 and dist > 0:
                print("distance smaller than 50")
                if (rect_smaller):
                ...
                if is_valid_placement((x_rect,y_rect,width_rect,depth_rect),extracted_positions, (rect_front, rect_left, rect_right, rect_bottom), room_width, room_depth):
                    # update placed objects
                    x_rect = new_x
                    y_rect = new_y
                    break
